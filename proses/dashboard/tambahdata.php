...
if (isset($_POST['startfiscal'])!=""  ){
  ...
  $query_dept = mysqli_query($link_yics,"SELECT * FROM dept_account") or die(mysqli_error($link_yics));
  if (mysqli_num_rows($query_dept)>0){
    while ($rows_dept = mysqli_fetch_assoc($query_dept)){
      ...
      if(isset($_POST[$id_dept])==$id_dept){
          $alokasi = $rows_dept['id_dept_account'];
          $budget = $_POST[$id_dept];
          $id = $startfiscal."_".$rows_dept['id_dept_account'];
...
<?php 
                 $morris = [];
                  $startfiscal = '2022-04-01';
                  $query_dept6 = mysqli_query($link_yics,"SELECT * FROM view_alokasi_budget WHERE startfiscal = '$startfiscal'") or die (mysqli_error($link_yics));
                  if  (mysqli_num_rows($query_dept6)>0){
                    while($rows_dept6=mysqli_fetch_assoc($query_dept6)){
                      $alokasi = $rows_dept6 ['department_account'];
                      $budget = $rows_dept6 ['budget'];
                      $morris[] = array('label' =>  $alokasi, 'value' =>  $budget);
                    }
